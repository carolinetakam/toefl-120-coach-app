...
| Post-login load recovery | PASS locally, production real login unverified | `vitest run`, `tsc --noEmit`, `eslint .`, and `next build --webpack` passed; local production HTTP smoke verified `/` and `/sign-in` return 200 and the build includes route/global recovery chunks. Production valid-account login still needs retest. |
