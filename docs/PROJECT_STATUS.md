...
- Auth-entry hardening now adds dedicated `/sign-in` and `/sign-up` Clerk pages and routes account buttons there instead of relying only on modal auth. After a user reported the page could not be loaded, a fallback `/?auth=sign-in` / `/?auth=sign-up` modal path was added locally and is pending deploy. Real-account login and signed-in sync remain unverified.
