...
- Auth-entry hardening now adds dedicated `/sign-in` and `/sign-up` Clerk pages and routes account buttons there instead of relying only on modal auth. This is deployed and production-smoked without credentials; real-account login and signed-in sync remain unverified.
