...
Current branch: `main`  
Current head when reviewed: `7e7aef5 fix: keep completed path from crashing restore`
...
1. **Production real-account login retest needed:** Dedicated auth pages are deployed and render correctly, and local recovery guards now exist for post-login load failures, but a real account must still complete login and verify signed-in Convex restore.
