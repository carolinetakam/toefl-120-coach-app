...
- Local signed-in auth-state priority now clears stale guest/safe-recovery/local-signout state when Clerk reports a signed-in user, then reopens the authenticated Convex restore path. This targets the screenshot issue where Clerk was signed in but the sidebar still showed `Guest learner`, `Guest mode`, and `Save Local`. Verified locally with focused regression tests, lint, typecheck after build regeneration, and production build; not yet deployed or retested with a real production account.
...
| Signed-in auth-state priority | PASS locally, production real login unverified | `components/coach-app.tsx` now clears `toefl-120-coach-guest-session`, safe-recovery mode, and local sign-out state when Clerk reports a signed-in user, then resets sync readiness so Convex restore can run; focused regression tests, focused lint, `next build --webpack`, and `tsc --noEmit` after build regeneration passed. Production deployment and real-account retest remain required. |
