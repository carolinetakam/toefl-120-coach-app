...
- Auth-entry hardening now adds dedicated `/sign-in` and `/sign-up` Clerk pages and routes account controls there. After user reports that auth still did not move forward, app-shell `Log In` / `Create Account` controls were changed to real links instead of client `router.push()` button handlers. Real-account login and signed-in sync remain unverified.
...
| Auth-entry link hardening | LOCAL PASS / PROD PENDING | Local build/type/focused lint passed; local production smoke verified `Log In` and `Create Account` render real `/sign-in` and `/sign-up` links. Production valid-account login still needs retest after deploy. |
