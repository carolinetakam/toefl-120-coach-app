...
Last updated: 2026-06-16 02:28 KST
Repo: `/Users/carolinetakam/Documents/apps/toefl-120-coach-app-only`  
Production URL: `https://score120coach.com`  
Current branch: `main`  
Current head when reviewed: `main` after auth-entry visible render guard deployment
...
- Cloud-restore render guard now checks sanitized Convex snapshots against the top-level app derivations before replacing the currently renderable workspace. This targets the follow-up issue where production briefly showed signed-in diagnostic/workspace state and then switched to the root recovery page. Commit `46b6173` was pushed and force-deployed to Vercel production; route/readiness smoke passed. Affected real account retest remains required.
- Signed-in progress restore key fix now looks up Convex users by Clerk token identifier first, then subject/email, and patches the current token identifier onto the matched user. This targets logout/login sessions that return a default `Learner` profile because the app cannot find the existing saved app state. The client also refuses to mark an empty cloud restore as `Save Synced` or autosave blank default progress over cloud. Convex and Vercel production were deployed on 2026-06-16; real affected-account retest remains required.
- Auth boot/restore order now redirects unauthenticated root sessions to `/sign-in`, sends explicit logout to `/sign-in`, and blocks personalized app content behind a signed-in restore-loading state until cloud progress restore completes. Clean production Chromium verified opening `/` while logged out lands on `/sign-in` with Clerk fields and does not show default `Learner` profile setup. Real affected-account login restore remains required.
- Local auth wrapper/guest-routing fix now keeps Clerk `/sign-in` and `/sign-up` controls visible/clickable by removing the hidden absolute Clerk mount and replacing fallback/loading UI with passive helper text below the Clerk surface. Auth-page `Continue as guest` links now route to `/?guest=1`, and the root app starts the existing guest mode from that query before cleaning the URL. Automated gates pass locally; deployment and real production Clerk/factor-two retest remain required.
- Logout return diagnosis is recorded in `docs/implementation-reports/2026-06-16-logout-return-diagnosis.md`. No code was changed. The current evidence points to a route race rather than a localStorage key: custom logout sets a React-only local signout flag before Clerk `signOut()` resolves, the app redirects to `/sign-in` as unauthenticated, and Clerk can still have an active session long enough for `/sign-in` to return to `/`. Active-browser JavaScript capture was blocked by browser settings, so the next step is diagnostic tracing of `isSignedIn`, `userId`, `sessionId`, app mode, and storage before/during/after logout.
- Local logout sequencing fix is implemented in `docs/implementation-reports/2026-06-16-logout-signout-sequencing-fix.md`. `CoachApp` now keeps Clerk signed-in state authoritative until Clerk reports signed out, waits for `await signOut()` plus a short no-active-session snapshot check before setting the local signout flag or redirecting, removes competing Clerk `UserButton` logout surfaces from the app shell, and logs temporary `AUTH_LOGOUT_BEFORE`, `AUTH_LOGOUT_AFTER_SIGNOUT_RESOLVED`, and `AUTH_REDIRECT_BEFORE_SIGN_IN` traces. Focused auth tests, full Vitest, TypeScript, ESLint, and production build pass locally. Deployment and real founder-account logout acceptance remain required.
...
| Auth entry visible render guard | PASS for visible production auth form, real login unverified | `components/auth-entry-panel.tsx` adds a visible loading/recovery panel while Clerk mounts; focused auth-route test, full Vitest, `tsc --noEmit`, `eslint .`, and `next build --webpack` passed; `npx vercel --prod --yes` deployed `dpl_AfM1t5k7x2cwjP6VEqDr2WFsuQu8` and aliased `score120coach.com`; production desktop Chromium verified clicking `Log In` reaches `/sign-in` with Clerk email/password inputs and no console/network failures; mobile Chromium verified `/sign-in` and `/sign-up` Clerk forms render with no horizontal overflow. Production valid-account login still needs retest. |
...
| Cloud restore render guard | PASS for deployment/static smoke, production real login unverified | `46b6173` runs a render-safety guard against sanitized Convex state before `setState(remoteState)`; focused recovery/render tests, full Vitest, focused lint, `tsc --noEmit`, and `next build --webpack` passed; `npx vercel --prod --yes` deployed `dpl_2fWZUk8pCM22o5puoRJzeYX4MDCv` and aliased `score120coach.com`; production readiness returned `ready:true`; public/auth routes returned HTTP 200; live bundle contains the restore guard. Affected-account retest remains required. |
| Signed-in progress restore key | PASS for deploy/static guard, affected-account retest unverified | Convex user lookup now falls back from token identifier to subject/email, then patches the matched user with the current token identifier; the client prevents empty cloud restore from syncing the default `Learner` profile; focused restore/blank-overwrite tests, full Vitest, `tsc --noEmit`, focused lint, and `next build --webpack` passed; Convex deployed to `brainy-chicken-240` with `users.by_subject` and `users.by_email`; Vercel deployment `dpl_23ddCsEKynt7SYfk5RKne43NbSCL` aliased `score120coach.com`; readiness and public/auth route smoke passed. Real logout/login restore still needs retest. |
| Auth boot restore order | PASS for logged-out root redirect and restore-loading guard, affected-account retest unverified | Root unauthenticated sessions now redirect to `/sign-in`, explicit logout redirects to `/sign-in`, and authenticated restore hides default `Learner` content until sync completes; focused auth/restore tests, full Vitest, `tsc --noEmit`, focused lint, and `next build --webpack` passed; Vercel deployment `dpl_FoV4GAKYKjm3zDTJLaG2nuvYc7sN` aliased `score120coach.com`; production clean Chromium verified `/` lands on `/sign-in` with Clerk fields and no default `Learner` app state; readiness/routes smoke passed. Real saved-progress restore still needs retest. |
| Auth wrapper / guest routing | PASS locally, production real login unverified | Clerk auth pages no longer hide the Clerk mount behind app fallback UI; helper text is passive below the Clerk surface; auth-page guest links use `/?guest=1`; root unauthenticated `guest=1` starts existing guest mode and cleans the URL. Focused auth/recovery tests, full Vitest, `tsc --noEmit`, `eslint .`, and `next build --webpack` passed. Production deploy and real password/factor-two retest remain required. |
| Logout signout sequencing | PASS locally, production founder-account retest required | `CoachApp` no longer enters unauthenticated mode while Clerk still reports signed in; custom logout awaits `signOut()` and confirms no active Clerk session before local signed-out cleanup and `/sign-in` redirect; competing `UserButton` logout surfaces were removed; temporary console traces capture before logout, after signout resolves, and before redirect. Focused auth tests, full Vitest, `tsc --noEmit`, `eslint .`, and `next build --webpack` passed. |
...
1. **Production real-account login retest needed:** Dedicated auth pages are deployed, route from the app `Log In` link, and visibly render Clerk forms in desktop/mobile Chromium. Logged-out root now redirects to `/sign-in`, explicit logout returns to `/sign-in`, and authenticated restore hides default `Learner` content while cloud progress loads. Local recovery guards exist for post-login load failures, the mixed Clerk/app auth-state issue has a targeted deployed patch, and Convex user restore lookup now falls back to subject/email before creating a blank user path. A real account must still complete logout/login and verify signed-in Convex restore after deployment.
