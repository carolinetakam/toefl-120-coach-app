...
Addendum 2026-06-16 02:05 KST: after production testing found Clerk password reaches `/sign-in/factor-two` but app fallback/loading copy may remain visible and intercept Continue clicks, `docs/implementation-reports/2026-06-16-auth-wrapper-guest-routing.md` records a narrow local fix. `components/auth-entry-panel.tsx` no longer hides Clerk behind an absolute app fallback; helper copy is passive below the Clerk mount and the old `Sign-in form is still loading` state is removed once any Clerk step renders. `/sign-in` and `/sign-up` guest links now route to `/?guest=1`; `CoachApp` starts the existing guest mode from that query and cleans the URL. Full automated gates pass locally. Deployment and real production password/factor-two plus guest-link retest remain required.

Addendum 2026-06-16 02:21 KST: after the user stopped further auth fixes and requested diagnosis of `logout -> brief redirect -> return to app`, `docs/implementation-reports/2026-06-16-logout-return-diagnosis.md` records the current finding. No code was changed. Browser JavaScript inspection of the active founder Chrome/Safari tabs was blocked by disabled Apple Events JavaScript, and shell storage search did not find a TOEFL/Clerk localStorage or sessionStorage key forcing app re-entry. Source and production bundle inspection point to a route race: the custom logout sets React-only `signedOutLocally` before Clerk `signOut()` resolves, `authState` becomes `unauthenticated`, the unauthenticated redirect effect sends the browser to `/sign-in`, and `/sign-in` can return to `/` while the Clerk session is still active. Next action must be diagnostic tracing of Clerk/app state around that transition, not another auth architecture change.

Addendum 2026-06-16 02:28 KST: after the diagnosis was accepted, `docs/implementation-reports/2026-06-16-logout-signout-sequencing-fix.md` records the smallest local fix. `CoachApp` now treats Clerk signed-in state as authoritative until Clerk reports signed out, awaits `signOut()` and confirms no active Clerk session before setting `signedOutLocally`, clearing local TOEFL state, or redirecting, removes competing Clerk `UserButton` logout surfaces, and adds temporary logout/redirect console traces. Focused auth tests, full Vitest, TypeScript, ESLint, and production build pass locally. Next action: deploy, then run the founder-account acceptance path `Login -> Logout -> /sign-in -> refresh stays /sign-in -> open / redirects /sign-in` and confirm the traces show no active Clerk session before redirect.
